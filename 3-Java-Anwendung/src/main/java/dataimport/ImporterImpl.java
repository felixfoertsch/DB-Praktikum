...
public class ImporterImpl implements Importer {
    ...
    /**
     * Main method of the dataimport.Importer. Imports a folder of CSV files into an University object that contains all the data.
     *
    ...
    public void checkMultiplicities(Universitaet universitaet) {
        ...
        for (Klausur k : universitaet.getKlausuren().values()) {
            if (k.getAufgaben().size() < 6 || k.getAufgaben().size() > 9) {
                aufgabenViolation = aufgabenViolation.concat(String.format("%-23s%-34s%s%n", "Violation of dataimport: " + k.getAufgaben().size() + " ", "Aufgabe in Klausur " + k.getKlausurNr() + ".", "Should be 6-9."));
            }

            if (k.getOrte().size() < 1 || k.getOrte().size() > 3) {
                ortViolation = ortViolation.concat(String.format("%-23s%-33s%s%n", "Violation of dataimport: " + k.getOrte().size() + " ", "Orte in Klausur " + k.getKlausurNr() + ".", "Should be 1-3."));
            }
        }
        ...
        for (Veranstaltung v : universitaet.getVeranstaltungen().values()) {
            ...
            if (g.getUebungen().size() < 4 || g.getUebungen().size() > 5) {
                System.out.printf("%-22s%-33s%s%n", "Violation of dataimport: " + g.getUebungen().size() + " ", "Uebungen in Grundvorlesung " + g.getName() + " (" + g.getKennung() + ").", "Should be 4-5.");
            }
        }
    }
